...
int main(int argc,char* argv[]){
  ...
  */
  ...
  int N;
  int range = 10;
  arma::vec timesteps = arma::linspace(2,5,range);
  timesteps.print();
  ...
  */
  ...
  for (int n=0; n < range; n++){
    ...
    filename = "test/eulerdat/orbitN" + nstr + csv;
    ...
    filename = "test/eulerdat/distanceN" + nstr + csv;
  ...
  for (int n=0; n < range; n++){
    ...
    // setting up filename
    nstr = std::to_string(n);
    filename = "test/vvdat/orbitN" + nstr + csv;
    // saving orbit positions
    ss.pos_evo.slice(1).save(filename, arma::csv_ascii);
